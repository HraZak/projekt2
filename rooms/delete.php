...
final class CurrentPage extends BaseDBPage
{
    ...
    protected function setUp(): void
    {
        ...
        if ($this->state == self::STATE_PROCESSED) {
            ...
            $stmt = $this->pdo->prepare('DELETE FROM `key` WHERE room=:roomId');
    ...
    protected function body(): string
    {
        if ($this->state == self::STATE_PROCESSED) {
            ...
            if ($this->result == self::RESULT_SUCCESS) {
                return $this->m->render("roomSuccess", ['message' => "Room deleted successfully.", 'login' => $_SESSION['login']]);
            } else {
                return $this->m->render("roomFail", ['message' => "Room deletion failed.", 'login' => $_SESSION['login']]);
            }
        }
        return "";
    }
